...
def valid_item(item):
    ...
    for key,value in item.items():
        ...
        if key == 'item_name':
            ...
            # Nothing immediate to do
            pass
        elif key == 'minimum_stock':
            # Same logic as count
            minstock_is_valid = valid_count(value)
            if not minstock_is_valid == "OK":
                return minstock_is_valid
        else:
            return "Format: Illegal JSON key name. Accepted tags are: "+\
                    "item_name, count, model_number, description, minimum_stock, is_asset, tags, and custom_fields."
    #print(str(keys))
    if not 'item_name' in keys:
        return "Format: Need item_name in JSON Item data."
    if 'is_asset' in keys and 'count' in keys:
        return "Format: count field should not be specified for an Asset. "+item.get('item_name','')
    return "OK"
...
def valid_customs(custom_fields,is_asset):
    ...
    # If Item is an Asset, CFs need to not be per-asset.
    for cf in custom_fields:
        ...
        try:
            ...
            if is_asset and existing_cf.per_asset:
                return "Format: Only non-per-asset custom fields can be specified for an asset item. "+cf_name
...
def save_items(items):
    for item in items:
        ...
        item_instance = Item(item_name=item['item_name'],name_unique_check=item['item_name'],\
            model_number=item.get('model_number',""),minimum_stock=item.get('minimum_stock',0),\
            description=item.get('description',""),count=item.get('count',0),\
            is_asset=item.get('is_asset',False))
